...
* **is_a?**
  * синонім з `kind_of?`
  * `a.kind_of? A == true`, якщо `a` - екземпляр класу `A`
  * `b.kind_of? A == true`, якщо `b` - екземпляр потомка класу `A`
  * `a.kind_of? ModA == true`, якщо `a` - екземпляр класу, в котрому через `prepend` чи `include` додано модуль `ModA`
  * `b.kind_of? ModA == true`, якщо `b` - екземпляр потомка, в батька котрого через `prepend` чи `include` додано модуль `ModA`
* **kind_of?**
  * синонім з `is_a?`
  * працює так само

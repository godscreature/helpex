...
* `p`
  * для виводу використовує матод `inspect`, котрий є в любого об'єкта
  * виводить те, що передали в більш говорящому вигляді
...
* `require`
  ...
  * існує два способи форматування рядка — або як абсолютний шлях, або як скорочене ім’я
* `require_relative`
